...
/**
 * binary search tree custom implementation using {@link Collection} interface
 *
 * @param <T> the type of objects that can be stored in binary search tree
 * @author a1rick
 */
public class BinarySearchTree<T extends Comparable<T>> implements Collection<T> {
    ...
    public Iterator<T> iterator() {
        return new BinarySearchTreeIterator<>(root, root != null ? findMin(root) : null);
    }
    ...
    private static final class BinarySearchTreeIterator<E extends Comparable<E>> implements Iterator<E> {

        private final Node<E> root;
        private Node<E> current;

        private BinarySearchTreeIterator(Node<E> root, Node<E> min) {
            this.root = root;
            this.current = min;
        }
